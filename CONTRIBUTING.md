# 贡献指南 / Contributing Guide

本文档为后续开发者提供完整的开发流程指南，基于第一性原理设计。

## 📐 第一性原理：项目核心设计

### 核心目标
帮助用户理解金融体系中**市场、机构、工具、宏观经济**之间的**复杂关系**。

### 五大领域 (Domains) - v2.0 架构
| 领域 | Domain | 说明 |
|------|--------|------|
| 金融市场 | `markets` | 一级/二级市场、股票/债券/衍生品市场等 |
| 金融机构 | `institutions` | 央行、商业银行、投行、交易所、监管机构等 |
| 金融工具 | `instruments` | 股票、债券、期权、期货、结构化产品等 |
| 宏观经济 | `macro` | 货币政策、财政政策、经济指标、经济周期等 |
| **金融基础设施** | `infrastructure` | 支付系统/清算/数据服务/跨境通道 |

### 三层层级 (Entity Levels)
| 层级 | 最低关系数 | 说明 | 示例 |
|------|-----------|------|------|
| L1 | 5 条 | 顶级概念 | 股票市场、中央银行、期货 |
| L2 | 3 条 | 二级分类 | A股、沪深300、国债 |
| L3 | 2 条 | 具体实例 | 工商银行、标普500、比特币 |

### 18 种关系类型 (v2.0 架构)
```typescript
type RelationType =
  // === 原有 11 种 ===
  | 'regulates'        // 监管 (监管机构→被监管者)
  | 'issues'           // 发行 (机构→工具)
  | 'trades'           // 交易 (机构↔市场/工具)
  | 'invests'          // 投资 (机构→工具/市场)
  | 'influences'       // 影响 (宏观层面间接影响)
  | 'depends_on'       // 依赖 (B依赖A才能运作)
  | 'derives_from'     // 衍生 (衍生品→标的)
  | 'competes_with'    // 竞争 (同类实体，双向)
  | 'cooperates_with'  // 合作 (业务协作，双向)
  | 'provides'         // 提供 ⚠️ 已废弃，限制使用
  | 'uses'             // 使用 (使用他人服务)
  // === 新增 7 种 (v2.0) ===
  | 'hosts'            // 托管/场所 (交易所→市场)
  | 'offers'           // 提供产品 (基金→投资者)
  | 'enables'          // 赋能 (基础设施→能力)
  | 'clears'           // 清算 (清算所→交易)
  | 'settles'          // 结算 (托管行→证券)
  | 'lists'            // 上市 (交易所→证券)
  | 'benchmarks';      // 定价基准 (LIBOR→利率产品)
```

> ⚠️ **关系类型使用规则**
> - `provides` **已废弃**，用于父→子层级，跨域关系请用 hosts/offers/enables
> - 优先使用精确类型：hosts(场所), offers(产品), enables(基础设施)
> - 单一类型不超过 30%，每种类型至少占 2%

---

## 🔧 开发环境设置

```bash
# 克隆并安装
git clone https://github.com/kpictor/Finance-map.git
cd Finance-map && npm install

# 启动开发服务器
npm run dev

# 运行数据验证 (重要！)
npm run validate

# 构建（自动验证）
npm run build
```

---

## 🚨 新实体添加工作流（7 阶段）

### 阶段 1：概念评估 (Gate)

| 问题 | 必须回答 |
|------|----------|
| 是金融体系构成要素还是观察工具？ | 只添加构成要素 |
| 是否已被现有实体覆盖？ | 避免重复 |
| 应该是什么层级 (L1/L2/L3)？ | 决定关系密度要求 |
| 属于哪个领域？ | 4 选 1 |
| 父节点是什么？ | L2/L3 必填 |

---

### 阶段 1.5：生态系统完成 ⚡（关键新增）

> **核心原则**：当用户提到一个概念时，应主动识别并添加完整的同类生态系统，并判断是否应该添加

| 问题 | 示例 |
|------|------|
| 这个实体有哪些全球同类？ | SGE → LBMA, COMEX, LME, TOCOM |
| 是否需要添加地区性同类？ | 美国/欧洲/亚太 各区域代表 |
| 是否需要添加上下游？ | 交易所 → 清算所 |

**示例**：用户说"添加上海黄金交易所"
- ❌ 错误做法：只添加 SGE
- ✅ 正确做法：添加全球贵金属交易所生态系统
  - SGE（亚洲）
  - LBMA（欧洲，黄金OTC）
  - COMEX（美国，黄金期货）
  - LME（欧洲，基本金属）
  - TOCOM（亚太，商品期货）

---

### 阶段 2：关系矩阵分析 ⚡（核心）

**对于每个新实体 X，必须系统遍历全部 18 种关系类型：**

#### 关系类型决策树（新增类型优先）

```
描述实体间关系时，按以下顺序判断：

1. 是否是"场所提供"？ → hosts (交易所→市场)
2. 是否是"产品/服务提供"？ → offers (机构→投资者)
3. 是否是"基础设施赋能"？ → enables (SWIFT→支付)
4. 是否是"清算"？ → clears (清算所→交易)
5. 是否是"结算"？ → settles (托管行→证券)
6. 是否是"上市"？ → lists (交易所→股票)
7. 是否是"定价参考"？ → benchmarks (LIBOR→利率)
8. 是否是"父→子层级"？ → provides (仅限层级包含)
9. 以上都不是 → 使用原有 10 种类型
```

#### 完整关系矩阵

| 关系类型 | X 作为 Source（X→?）| X 作为 Target（?→X）|
|----------|---------------------|---------------------|
| **hosts** | X 托管什么市场？ | 谁托管 X？ |
| **offers** | X 提供什么产品？ | 谁向 X 提供产品？ |
| **enables** | X 赋能什么？ | 谁赋能 X？ |
| **clears** | X 清算什么？ | 谁清算 X？ |
| **settles** | X 结算什么？ | 谁结算 X？ |
| **lists** | X 上市什么？ | X 在哪上市？ |
| **benchmarks** | X 作为什么的基准？ | X 参考什么基准？ |
| regulates | X 监管谁？ | 谁监管 X？ |
| issues | X 发行什么？ | 谁发行 X？ |
| trades | X 交易什么？ | 谁交易 X？ |
| invests | X 投资什么？ | 谁投资 X？ |
| influences | X 影响谁？ | 谁影响 X？ |
| depends_on | X 依赖谁？ | 谁依赖 X？ |
| derives_from | X 从何衍生？ | 什么从 X 衍生？ |
| competes_with | X 与谁竞争？ | （双向）|
| cooperates_with | X 与谁合作？ | （双向）|
| provides | X 包含什么子类？ | X 属于哪个父类？ |
| uses | X 使用什么？ | 谁使用 X？ |

> **每个格子都必须显式回答**（即使答案是"无"）

---

### 阶段 3：涟漪效应分析

检查现有相关实体是否需要连接到新实体：

```
对于新实体 X 所在类别的每个现有实体 Y：
  - Y 与 X 是否有竞争/合作关系？
  - Y 是否使用/依赖 X（或反之）？
```

---

### 阶段 4：密度检查

| 层级 | 最低关系数 |
|------|-----------|
| L1 | ≥ 5 |
| L2 | ≥ 3 |
| L3 | ≥ 2 |

---

### 阶段 5：类型平衡检查

- 单一关系类型不超过 **40%**
- 每种类型至少占 **2%**

---

### 阶段 6：验证与提交

```bash
npm run validate
git add . && git commit -m "feat: add XXX with full relationship matrix"
git push
```

---

## ⚡ 数据验证系统（7 层）

| 层级 | 名称 | 验证内容 |
|------|------|----------|
| L1 | 引用完整性 | ID唯一性、关系引用有效性 |
| L2 | 层级一致性 | 子节点层级 > 父节点层级 |
| L3 | 领域语义 | 子父领域一致性 |
| L4 | 关系约束 | 关系类型语义规则 |
| L5 | 图结构 | 孤立节点检测 |
| **L6** | **关系密度** | **L1≥5, L2≥3, L3≥2** |
| **L7** | **关系平衡** | **单类型≤40%, 每类型≥2%** |

---

## 📋 新增实体检查清单模板

```markdown
# 新增实体：[实体名称]

## 阶段 1：概念评估
- [ ] 是金融体系构成要素
- [ ] 未被现有实体覆盖
- [ ] 层级：L___
- [ ] 领域：markets / institutions / instruments / macro / infrastructure
- [ ] 父节点：_________

## 阶段 1.5：生态系统
- [ ] 识别全球同类：_________
- [ ] 识别区域同类：_________
- [ ] 是否需要添加上下游：_________

## 阶段 2：关系矩阵 (18种)
| 关系类型 | X→? | ?→X |
|----------|-----|-----|
| **hosts** | | |
| **offers** | | |
| **enables** | | |
| **clears** | | |
| **settles** | | |
| **lists** | | |
| **benchmarks** | | |
| regulates | | |
| issues | | |
| trades | | |
| invests | | |
| influences | | |
| depends_on | | |
| derives_from | | |
| competes_with | | |
| cooperates_with | | |
| provides | | |
| uses | | |

## 阶段 3：涟漪效应
需检查的现有实体：
- [ ] _________
- [ ] _________

## 阶段 4-6：验证
- [ ] 关系数 ≥ 最低要求 (L1≥5, L2≥3, L3≥2)
- [ ] npm run validate 通过
- [ ] 无使用 provides 的跨域关系（用 hosts/offers/enables 替代）
```

---

## 📄 License

MIT
